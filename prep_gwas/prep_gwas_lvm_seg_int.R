...
create(trait="lvmi_seg_adjusted_int",data=seg_lvm)

# Exclusions list for BOLT
## Read processed phenotype
seg_lvm <- fread(file='lvmi_seg_adjusted_int.tsv')
...
write.table(exclusions,file='lvmi_seg_exclusion_int.tsv',sep="\t",col.names =T,row.names = F,quote = F)
