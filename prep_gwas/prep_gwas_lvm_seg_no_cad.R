...
# Merges
setkey(mi,sample_id); setkey(hf,sample_id); setkey(seg_lvm,sample_id)
seg_lvm[mi,":="(has_mi = i.has_disease, mi_date = i.censor_date)]
seg_lvm[hf,":="(has_hf = i.has_disease, hf_date = i.censor_date)]

# Prev variables
seg_lvm[,":="(prev_mi = ifelse(c(has_mi == 1 & (mi_date <= mri_date)),1,0),
              prev_hf = ifelse(c(has_hf == 1 & (hf_date <= mri_date)),1,0))]
seg_lvm <- seg_lvm[c(prev_mi==0 & prev_hf==0)]
