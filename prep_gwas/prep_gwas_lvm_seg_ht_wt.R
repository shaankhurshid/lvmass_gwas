...
seg_lvm[ht0,ht := i.ht_unified]
seg_lvm[wt0,wt := i.wt_unified]
...
create<-function(trait,exclude_all_both=NULL,exclude_all_cases=NULL,exclude_all_controls=NULL,
  ...
  #######
  ## Choose columns
  pheno<-ab[,c("sample_id",trait,"age_at_mri",rownames(s1)[1:5],"array_UKBB","male","ht","wt"),with=F]
  ## Format for PLINK
...
seg_lvm <- fread(file='lvm_seg_adjusted.tsv')
...
write.table(exclusions,file='lvm_seg_exclusions.tsv',sep="\t",col.names =T,row.names = F,quote = F)
