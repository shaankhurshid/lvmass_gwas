...
seg_lvm <- fread(file='seg_lvm_nfmass.csv')
...
create(trait="lvmi_nfmass",data=seg_lvm)

# Exclusions list for BOLT
## Read processed phenotype
seg_lvm <- fread(file='lvmi_nfmass.tsv')
...
write.table(exclusions,file='lvmi_nfmass_exclusion.tsv',sep="\t",col.names =T,row.names = F,quote = F)
